...
| `prs` | Alias of `mrs` — the same array, for pipeline portability. |
...
| `draft` | Optional, default `false` | GitLab has no draft-release concept — a `true` value logs a warning and is ignored. |
| `prerelease` | Optional, default `false` | GitLab has no native prerelease flag — appends ` (pre-release)` to the release name instead. |
...
| `file` | Optional, default `moonlit.env` | Target file, appended to (not truncated). Must be a relative path within the working directory — an absolute path or one containing `..` fails. |
